...
from jepa_src.models.vision_transformer import VIT_EMBED_DIMS as JEPA_EMBED_DIMS
from jepa_src.models.attentive_pooler import AttentivePooler
...
class PerceptWrapper(nn.Module):
	def __init__(self, backbone, call_fn_name):
		super().__init__()
		self.backbone = backbone
		self.call_fn = getattr(self.backbone, call_fn_name)
...
class TCP(nn.Module):
	...
	def __init__(self, config):
		...
		jepa_config = self.config.__dict__.get("jepa_config_file")
		if jepa_config:
...
class TCP_JEPA(nn.Module):
	...
	def __init__(self, config):
		...
		jepa_config = getattr(self.config, "jepa_config_file",)
		...
		# self.perception.backbone.freeze_encoder()
		
		jepa_embed_dim = getattr(self.config, "jepa_embed_dim")
		...
		self.join_traj = nn.Sequential(
							nn.Linear(jepa_embed_dim + 128, 512),
							# nn.Linear(128+1000, 512),
							nn.GELU(),
							nn.Linear(512, 512),
							nn.GELU(),
		...
		self.join_ctrl = nn.Sequential(
							nn.Linear(128 + jepa_embed_dim, 512),
							nn.GELU(),
							nn.Linear(512, 512),
							nn.GELU(),
							nn.Linear(512, 256),
		...
		self.decoder_ctrl = nn.GRUCell(input_size=256+4, hidden_size=256)
		# self.output_ctrl = nn.Sequential(
		# 		nn.Linear(256 + jepa_embed_dim, 256),
		# 		nn.GELU(),
		# 		nn.Linear(256, 256),
		# 		nn.GELU(),
		# 	)
		self.output_ctrl = nn.Sequential(
				nn.Linear(256 + 196, 256),
				nn.GELU(),
				nn.Linear(256, 256),
				nn.GELU(),
			)
	...
	def forward(self, img, state, target_point):
		# feature_emb, cnn_feature = self.perception(img)
		img_embedding = self.perception(img)
		feature_emb = self.attn_pool(img_embedding).squeeze(1)
		outputs = {}
		outputs['pred_speed'] = self.speed_branch(feature_emb)
		measurement_feature = self.measurements(state)
